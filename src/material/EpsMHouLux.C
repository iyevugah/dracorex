...
template <bool is_ad>
EpsMHouLuxTempl<is_ad>::EpsMHouLuxTempl(const InputParameters & parameters)
...
template <bool is_ad>
void
EpsMHouLuxTempl<is_ad>::computeStressInitialize(const GenericReal<is_ad> & /*effective_trial_stress*/,
...
template <bool is_ad>
void
EpsMHouLuxTempl<is_ad>::updateDamageParam(const GenericReal<is_ad> & effective_trial_stress,
...
ScalarType
EpsMHouLuxTempl<is_ad>::computeResidualInternal(const GenericReal<is_ad> & effective_trial_stress,
                                              const ScalarType & scalar)
{
  ...
  const ScalarType creep_rate = (stress_delta / ((1.0 -_damage_param[_qp]) * etaMax));
...
template <bool is_ad>
GenericReal<is_ad>
EpsMHouLuxTempl<is_ad>::computeDerivative(const GenericReal<is_ad> & effective_trial_stress,
                                        const GenericReal<is_ad> & scalar)
{
  ...
  const GenericReal<is_ad> creep_rate_derivative =
                  (_three_shear_modulus/((1.0-_damage_param[_qp]) * etaMax)) * ((stress_delta*_mvM)-1);
...
template <bool is_ad>
void
EpsMHouLuxTempl<is_ad>::resetIncrementalMaterialProperties()
{
...
template <bool is_ad>
bool
EpsMHouLuxTempl<is_ad>::substeppingCapabilityEnabled()
{
...
template Real EpsMHouLuxTempl<false>::computeResidualInternal<Real>(const Real &, const Real &);
template ADReal EpsMHouLuxTempl<true>::computeResidualInternal<ADReal>(const ADReal &,
                                                                     const ADReal &);
template ChainedReal
EpsMHouLuxTempl<false>::computeResidualInternal<ChainedReal>(const Real &, const ChainedReal &);

template ChainedADReal
EpsMHouLuxTempl<true>::computeResidualInternal<ChainedADReal>(const ADReal &, const ChainedADReal &);
